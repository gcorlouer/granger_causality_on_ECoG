...
def single_fc_stat(
    ...
    for icomp in range(ncomp):
        ...
        for i in range(n):
            for j in range(n):
                z[i, j, icomp], pval[i, j, icomp] = ranksums(
...
def plot_single_trial_gfc(
    ...
    for s in range(nsub):
        ...
        reader = EcogReader(args.data_path, subject=subject)
        df_visual = reader.read_channels_info(fname="visual_channels.csv")
        populations = parcellation_to_indices(
