...
class EcogReader:
    ...
    def read_ecog(self, run=1, condition="stimuli"):
        ...
        # Write file path and name for raw or bipolar rereferenced.
            ...
            fpath = self.path.joinpath(
                "source_data", "iEEG_10", "subjects", self.subject, "EEGLAB_datasets"
            )
...
def drop_bad_chans(raw, q=99, voltage_threshold=500e-6, n_std=5):
    ...
    outliers_chans = pick_99_percentile_chans(
        raw, q=q, voltage_threshold=voltage_threshold
    )
    ...
    for i in range(nchan):
        for j in range(nobs):
            if X[i, j] >= top_percentile[i]:
                ...
                continue
        average_value_in_top_percentile[i] = (
            average_value_in_top_percentile[i] / count[i]
        )
    # Return channels whose average value in the top percentile is above voltage
    # threshold
    outliers_indices = np.where(average_value_in_top_percentile >= voltage_threshold)[
        0
    ].tolist()
...
class HfbExtractor:
    ...
    def extract_envelope(self, raw):
        ...
        raw_band = raw.copy().filter(
            l_freq=self.l_freq,
            h_freq=self.l_freq + self.band_size,
            phase=self.phase,
...
class Epocher:
    ...
    def epoch(self, raw):
        ...
        if self.condition == "Rest":
            ...
            # removed signal
            events_1 = mne.make_fixed_length_events(
                raw, id=32, start=5, stop=205, duration=2, first_samp=False, overlap=0.0
            )
            events_2 = mne.make_fixed_length_events(
                raw,
                id=32,
                start=215,
                stop=415,
    ...
    def baseline_rescale(self, epochs):
        ...
        times = epochs.times
        # Baseline rescaling
        A = 10 * mne.baseline.rescale(
            A, times, baseline=(self.tmin_baseline, self.tmax_baseline), mode=self.mode
        )
        # Create epoch object from array
        hfb = mne.EpochsArray(
            A, epochs.info, events=events, event_id=event_id, tmin=self.t_prestim
        )
        return hfb
    ...
    def extract_baseline(self, epochs):
        ...
        """
        baseline = epochs.copy().crop(
            tmin=self.tmin_baseline, tmax=self.tmax_baseline
        )  # Extract prestimulus baseline
...
class VisualDetector:
    ...
    def multiple_wilcoxon_test(self, A_postim, A_prestim):
        ...
        for i in range(0, nchans):
            ...
            se = np.sqrt(
                n * (n + 1) * (2 * n + 1) / 24
            )  # se of null (no correction for tie)
            w[i], pval[i] = spstats.wilcoxon(
                A_postim[:, i], A_prestim[:, i], alternative="greater"
            )
            z[i] = (w[i] - mn) / se
            z[i] = round(z[i], 2)
            # Unpaired test
    ...
    def compute_latency(self, visual_hfb, image_id, visual_channels):
        ...
        for i in range(len(visual_channels)):
            for t in range(np.size(A_postim, 2)):
                tstat[t] = spstats.ranksums(
                    A_postim[:, i, t], A_baseline[:, i], alternative="greater"
                )
                pval[t] = tstat[t][1]

            reject, pval_correct = fdrcorrection(
                pval, alpha=self.alpha
...
class VisualClassifier(VisualDetector):
    ...
    def classify_visual_chans(self, hfb, dfelec):
        ...
        group, category_selectivity = self.classify_face_place(
        ...
        for chan in visual_chan:
            ...
            visual_populations["brodman"].extend(
                dfelec["Brodman"].loc[dfelec["electrode_name"] == chan_name_split]
            )
            visual_populations["DK"].extend(
                dfelec["ROI_DK"].loc[dfelec["electrode_name"] == chan_name_split]
            )
            visual_populations["X"].extend(
                dfelec["X"].loc[dfelec["electrode_name"] == chan_name_split]
            )
            visual_populations["Y"].extend(
                dfelec["Y"].loc[dfelec["electrode_name"] == chan_name_split]
            )
            visual_populations["Z"].extend(
                dfelec["Z"].loc[dfelec["electrode_name"] == chan_name_split]
            )
            visual_populations["hemisphere"].extend(
                dfelec["hemisphere"].loc[dfelec["electrode_name"] == chan_name_split]
            )
    ...
    def classify_face_place(self, visual_hfb, face_id, place_id, visual_channels):
        ...
        A_face = self.crop_stim_hfb(
            visual_hfb, face_id, tmin=self.tmin_postim, tmax=self.tmax_postim
        )
        A_place = self.crop_stim_hfb(
            visual_hfb, place_id, tmin=self.tmin_postim, tmax=self.tmax_postim
        )
...
def prepare_condition_ts(
    ...
    for condition in conditions:
        ...
        indices = parcellation_to_indices(
            df_visual, parcellation="group", matlab=matlab
        )
...
def prepare_condition_scaled_ts(
    path,
    subject="DiAs",
    stage="preprocessed",
    matlab=False,
...
def parcellation_to_indices(visual_population, parcellation="group", matlab=False):
    ...
    for key in group:
        group_indices[key] = visual_population.loc[
            visual_population[parcellation] == key
        ].index.to_list()
...
def plot_multi_fc(
    fc, populations, s=2, sfreq=250, rotation=90, tau_x=0.5, tau_y=0.8, font_scale=1.6
):
    """
...
def single_pfc_stat(
    ...
    for icomp in range(ncomp):
        ...
        for i in range(n):
            for j in range(n):
                z[i, j, icomp], pval[i, j, icomp] = spstats.ranksums(
                    f[i, j, :], fb[i, j, :], alternative=alternative
                )
...
def category_ts(hfb, visual_chan, sfreq=250, tmin_crop=0.050, tmax_crop=0.250):
    ...
    for idx, cat in enumerate(condition):
        X, time = hfb_to_category_time_series(
            hfb,
            visual_chan,
            sfreq=sfreq,
            cat=cat,
...
def hfb_to_category_time_series(
    hfb, visual_chan, sfreq=250, cat="Rest", tmin_crop=0.5, tmax_crop=1.5
):
    """
    ...
    """
    hfb = category_hfb(
        hfb, visual_chan, cat=cat, tmin_crop=tmin_crop, tmax_crop=tmax_crop
    )
...
def category_hfb(hfb, visual_chan, cat="Rest", tmin_crop=-0.5, tmax_crop=1.5):
    ...
    # Epoch condition specific HFB
    hfb, events = epoch_condition(hfb, cat=cat, tmin=-0.5, tmax=1.75)
...
def ts_to_population_hfb(ts, visual_populations, parcellation="group"):
    ...
    populations_indices = parcellation_to_indices(
        visual_populations, parcellation=parcellation
    )
...
def read_roi(df_visual, roi="functional"):
    ...
    if roi == "functional":
        ...
        roi_idx = {
            "LO": roi_idx["ctx-lh-lateraloccipital"],
...
def plot_smvgc(f_roi, roi_idx, sfreq=250, x=40, y=0.01, font_scale=1.5):
    ...
    figure, ax = plt.subplots(
        n_roi, n_roi, sharex=True, sharey=True, figsize=(10, 8), dpi=80
    )
    for c in range(n_cdt):
        for i in range(n_roi):
            for j in range(n_roi):
                ax[i, j].plot(
                    freqs, f_roi[i, j, :, c], label=f"{conditions[c]}", linewidth=2.5
                )
...
def cross_subject_ts(
    ...
    for s in range(len(cohort)):
        subject = cohort[s]
        ecog = Ecog(cohort_path, subject=subject, proc=proc, stage=stage, epoch=epoch)
        hfb = ecog.read_ecog()
        df_visual = ecog.read_channels_info(fname=channels)
        visual_chan = df_visual["chan_name"].to_list()
        ts[s], time = category_ts(
            hfb, visual_chan, sfreq=sfreq, tmin_crop=tmin_crop, tmax_crop=tmax_crop
        )
...
def sliding_ts(
    ...
    for i in range(nwin):
        ...
        ts[i], time[i] = chan_specific_category_ts(
            picks,
            sub_id=sub_id,
            proc=proc,
            stage=stage,
...
def sliding_lfp(
    ...
    for i in range(nwin):
        tmin_crop = window[i][0]
        tmax_crop = window[i][1]
        ts[i], time[i] = chan_specific_category_lfp(
            picks,
            sub_id=sub_id,
            proc=proc,
            stage=stage,
...
def category_continous_sliding_ts(hfb, tmin=0, tmax=265, step=1, win_size=10):
    ...
    for i in range(ncat):
        ts[i], time = make_continuous_sliding_ts(
            hfb_cat[i], tmin=tmin, tmax=tmax, step=step, win_size=win_size
        )
