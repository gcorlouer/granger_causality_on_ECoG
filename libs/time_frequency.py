...
def compute_group_power(args, tf_args, subject="DiAs", group="R", condition="Face"):
    """
...
def plot_tf(fpath, subject="DiAs", vmax=25):
    ...
    for i, condition in enumerate(conditions):
        for j, group in enumerate(groups):
            power = (
                df["power"]
                .loc[df["subject"] == subject]
                .loc[df["condition"] == condition]
                .loc[df["group"] == group]
            )
            freqs = (
                df["freqs"]
            ...
            mesh = ax[i, j].pcolormesh(
                time, freqs, power, cmap="RdBu_r", vmax=vmax, vmin=-vmax
            )
