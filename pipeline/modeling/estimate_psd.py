...
from src.preprocessing_lib import (
    prepare_condition_scaled_ts,
    EcogReader,
    parcellation_to_indices,
)
...
parser.add_argument(
    "--preprocessed_suffix", type=str, default="_hfb_continuous_raw.fif"
)
...
def ts_to_psd(
    ts, indices, condition="Face", group="F", sfreq=250, fmax=100, bandwidth=6
):
    """
...
def average_psd(
    args, data_path, condition="Face", group="F", fmax=50, bandwidth=8, matlab=False
):
    """
...
for i, group in enumerate(visual_group):
    for c, condition in enumerate(conditions):
        ...
        evoked_hfa, sem_hfa, time, baseline = average_hfa(
            args, cohort, condition=condition, group=group
        )
