...
eeg_bands = ["[4 7]", "[8 12]", "[13 30]", "[32 60]", "[60 120]", "[0 62]"]  # EEG bands
...
eeg_bands_fname_dic = {
    ...
    "[60 120]": "hgamma",
    "[0 62]": "hfa",
}  # to safe fig file name

eeg_bands_fig_title_dic = {
    "[1 4]": "δ",
    "[4 7]": "θ",
    "[ 8 12]": "α",
    "[13 30]": "β",
...
def plot_TD_BU_zscore(
    F, cohort, data_path, cmap="PuOr", vmin=-5, vmax=5, tau_x=0.5, tau_y=0.8
):
    """
...
for band in eeg_bands:
    ...
    plot_TD_BU_zscore(
        F, cohort, data_path, cmap=cmap, vmax=vmax, vmin=vmin, tau_x=0.5, tau_y=0.8
    )
    figpath = Path(
