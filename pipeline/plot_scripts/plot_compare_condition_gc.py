...
def plot_single_zscore(
    per_subject_data, args, FC="single_GC", vmin=-3, vmax=3, tau_x=0.5, tau_y=0.8
):
    """
    ...
    for s, subject in enumerate(cohort):
        for c, comparison in enumerate(comparisons):
            ...
            z = per_subject_data[subject][0][0][comparison][0][0][FC][0][0]["zstat"][0][
                0
            ]["z"][0][0]
            zcrit = per_subject_data[subject][0][0][comparison][0][0][FC][0][0][
                "zstat"
            ][0][0]["zcrit"][0][0]
            sig = per_subject_data[subject][0][0][comparison][0][0][FC][0][0]["zstat"][
                0
            ][0]["sig"][0][0]
            ticks = ["R", "O", "F"]
...
def plot_group_zscore(
    cross_subject_data, FC="single_GC", vmin=-3, vmax=3, tau_x=0.5, tau_y=0.8
):
    comparisons = ["FaceRest", "PlaceRest", "FacePlace"]
    ncomp = len(comparisons)
    ...
    for c, comparison in enumerate(comparisons):
        ...
        zcrit = cross_subject_data[comparison][0][0][FC][0][0]["zstat"][0][0]["zcrit"][
            0
        ][0]
