...
df_postim = build_hfb_dataframe(
    hfb_postim, long_format=True, bad_condition=["-1", "-2"]
)
df_baseline = build_hfb_dataframe(
    baseline, long_format=True, bad_condition=["-1", "-2"]
)
...
g = sns.FacetGrid(
    df, col="channel", hue="condition", col_wrap=4, legend_out="True", palette=pal
)
g.map_dataframe(
    sns.histplot, x="value", hue="condition", stat="probability", palette=pal
)
g.add_legend()
