...
if ~exist('nx',     'var'), nx      = 1;           end % number of target variables
if ~exist('ny',     'var'), ny      = 1;           end % number of source variables
if ~exist('nz',     'var'), nz      = 0;           end % number of conditioning variables
...
if ~exist('rho',    'var'), rho     = [0.9,0.9];  end % spectral radii
if ~exist('wvar',   'var'), wvar    = [0.9,0.9];   end % var coefficients decay weighting factors
...
connectivity_matrix1 = [[0;1],[0;0]];
connectivity_matrix2 = [[0;1],[1;0]];

% Random VAR model  (same model for both conditions)
A1 = var_rand(connectivity_matrix1,p,rho(1),wvar(1));
V = corr_rand(n,rmi(1));
A2 = var_rand(connectivity_matrix2,p,rho(1),wvar(1));

% Generate 2 time conditions
X1 = var_to_tsdata(A1,V,m,N(1));
X2 = var_to_tsdata(A2,V,m,N(2));
...
% fname = 'test_permtest_vs_single_trial_different_model.mat';
