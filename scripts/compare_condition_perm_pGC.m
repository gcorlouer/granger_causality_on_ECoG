...
for s=1:nsub
    ...
    for c=1:nComp
            ...
            for i=1:2
                ...
                % Compute pairwise GC 
                F{i} = ss_to_pwcgc(A,C,K,V);
            end
            observedF = F{1} - F{2};
            X = cat(3, Xc{1},Xc{2});
            stat = compare_condition_pGC(X, 'obsStat', observedF, 'N', N, ...
                'Ns', Ns, 'morder', morder, 'ssmo',ssmo);
            pGC.(subject).(comparison_name).('z') = stat.z;
            pGC.(subject).(comparison_name).('sig') = stat.sig;
            pGC.(subject).(comparison_name).('pval') = stat.pval;
            pGC.(subject).(comparison_name).('zcrit') = stat.zcrit;
            pGC.(subject).indices = indices;
    end
end
