...
def plot_psd(args, cohort, fmin=0, fmax=100, bandwidth=2, adaptive=True):
    ...
    for s, subject in enumerate(cohort):
        ...
        for p, pop in enumerate(populations):
            for c, cdt in enumerate(conditions):
                ...
                ax[p,0].set_ylabel(f'{pop}, Power (dB)')
                ...
                ax[2,s].set_xlabel("Frequency (Hz)")
    fig.legend(handles, labels, loc='upper right')
    fig.suptitle('PSD of HFA', )
    plt.show()
...
fmax = 100
bandwidth = 2
plot_psd(args, cohort, fmin=fmin, fmax=fmax, bandwidth=bandwidth, adaptive=True)
