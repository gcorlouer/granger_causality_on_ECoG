...
# Signal dic
hfa_dic = {'suffix': '_hfb_continuous_raw.fif', 'signal': 'hfa',
           'log_transf': True, 'decim': 4}
lfp_dic = {'suffix': '_bad_chans_removed_raw.fif', 'signal': 'lfp',
           'log_transf': False, 'decim': 2}
if signal == 'hfa':
    signal_dic = hfa_dic
elif signal == 'lfp':
    signal_dic = lfp_dic
...
fig_path = Path('~','thesis','overleaf_project', 'figures', 'supplementary_methods_figures').expanduser()
...
for subject in cohort:
    ...
    parser.add_argument("--signal", type=str, default= signal_dic['signal']) # correspond to preprocessed_suffix
    ...
    ts = prepare_condition_ts(data_path, subject=args.subject, stage=args.stage, matlab = args.matlab,
    ...
    for condition in conditions:
        ...
        for i in range(n):
            ...
            ax.set_xticks([])
            ax.set_yticks([])
        plt.suptitle(cohort_dic[args.subject] + ' '  + f'distributions, visual {signal}, {condition} condition' )
