...
ecog = EcogReader(args.data_path, subject=args.subject, stage=args.stage,
                 preprocessed_suffix=args.preprocessed_suffix)
raw = ecog.read_ecog()

hfb = HfbExtractor(l_freq=args.l_freq, nband=args.nband, band_size=args.band_size, 
...
def test_extract_envelope(channel):
    ...
    raw_band = raw.copy().filter(l_freq=args.l_freq, h_freq=args.l_freq+args.band_size,
...
def test_mean_normalise():
    ...
    extractor = HfbExtractor(l_freq=args.l_freq, nband=args.nband, band_size=args.band_size, 
              l_trans_bandwidth= args.l_trans_bandwidth,
                    h_trans_bandwidth= args.h_trans_bandwidth, 
                    filter_length=args.filter_length, phase=args.phase,
                    fir_window=args.fir_window)
