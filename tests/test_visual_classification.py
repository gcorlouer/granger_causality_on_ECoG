...
parser.add_argument(
    "--preprocessed_suffix", type=str, default="_hfb_Stim_scaled-epo.fif"
)
...
def test_classify_face_place():
    ...
    group, category_selectivity = classifier.classify_face_place(
        hfb, face_id, place_id, visual_channels
    )
...
A_face = detector.crop_stim_hfb(
    hfb, face_id, tmin=args.tmin_postim, tmax=args.tmax_postim
)
A_place = detector.crop_stim_hfb(
    hfb, place_id, tmin=args.tmin_postim, tmax=args.tmax_postim
)
